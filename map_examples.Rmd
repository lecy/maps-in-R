...
R is an incredibly powerful tool for data analysis. Is an analytical platform, it has benefitted from the decision to make the software free, open-source, and easily accessible. As a result, it has been embraced by the academic and data sciece communities as useful way to develop and share new software. As a result, people have written thousands of programs for R that can be used for specialized or advanced analysis. The core R program is best understood as an operating system that is designed to run programs, called 'packages', written by members of the R community. This has allowed the platform to evolve rapidly through libraries of user-created tools that extend far beyond the original intent of the software.


Although R was created program primarily for statistical analysis, it has become a powerful engine for mapping and geographic analysis. This vignette is designed to introduce the reader to a few of the hundreds of [spatial packages](https://cran.r-project.org/web/views/Spatial.html) in R to give a sense of the scope and power of the available tools.
...
Fortunately R makes this process a little more palatable through the use of APIs. A data API is an easy way to query a database through a website address. Your query parameters are sent through web address, and your data is printed direclty in your browser or returned as a file download. We when call the API through R, we can read the dataset directly into the program.
...
The database query would then then be: http://api.datausa.io/api/csv/?show=geo&sumlevel=county&required=adult_smoking
...
For this map, let's grab a few variables related to public health: smoking, obesity, drunk driving deaths, and Medicare reiumbursement rates. Documentation for the API is available [here](https://gist.github.com/lecy/0aa782a873cd174573f32d243233ca5b). 
...
plot( 1:5, rep(1,5), ylim=c(0,2), xlim=c(0,6), yaxt="n", xaxt="n", bty="n", xlab="", ylab=""  )
points( 1:5, rep(1,5), pch=15, cex=8, col=col.ramp )
```
...
We have some data. We have a color scale. We have split the data into groups. We are not ready to create the map. Let's use an Albers projection instead of the default rectangular projection.
...
If we want to examine some trends in health patterns across the country we can plot multiple maps together. In this case, we can see that there is a greater prevelance of smoking and obesity in the South, but drunk driving seems to occur at equal rates across the country. Medicate reimbursement rates are also lower in the South than in other parts of the country.  
...
data.frame( BASIC=basic, AUGMENTED=head( addresses ) )
```
...
```{r}
head(   as.data.frame( downtown )[ , c(2:7,11,21)]   )
